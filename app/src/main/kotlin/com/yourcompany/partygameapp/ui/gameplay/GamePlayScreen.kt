                ...
                Scaffold(
                    ...
                    NavHost(
                        ...
                        composable(
                            ...
                            PreGameScreen(
                                ...
                                onStartGame = { gameMode, deckId, playerId, teamTime, maxSkips, playersJson ->
                                    // playerId will be 0 for HotPotato, actual ID for other modes
                                    sharedViewModel.playerId = playerId
                                    // For HotPotato, playerName will be updated in GamePlayViewModel or remain "Player" dummy for score screen
                                    sharedViewModel.playerName = preGameViewModel.playerList.value.find { it.id == playerId }?.name ?: "Player"

                                    val encodedPlayersJson = Uri.encode(playersJson)
                        ...
                        composable(route = Routes.AFTER_GAME) {
                            ...
                            LaunchedEffect(Unit) {
                                // Only save results for non-Hot Potato games (playerId > 0)
                                // Or if Hot Potato, save team scores differently
                                if (sharedViewModel.playerId != 0) { // Check if it's a "real" player
                            ...
                            AfterGameScreen(
                                ...
                                onPlayAgain = { navController.navigateUp() },
